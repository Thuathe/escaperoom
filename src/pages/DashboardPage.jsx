...
useEffect(() => {
  ...
  return (
    <div className="relative h-[100svh] bg-[#0d0f1a] overflow-hidden flex items-center justify-center">
      ...
      <motion.div
        className="absolute z-0 w-[500px] h-[500px] rounded-full border border-amber-300/20"
        transition={{ repeat: Infinity, duration: 60, ease: "linear" }}
        style={{
          boxShadow:
            "0 0 60px rgba(251,191,36,0.07), 0 0 120px rgba(251,191,36,0.05)",
        }}
      />
...
<motion.button
  ...
  className="w-[100px] h-[100px] rounded-full border-2 border-yellow-500 shadow-lg overflow-hidden bg-black/30 backdrop-blur-md transition-all relative"
  whileHover={{
    scale: 1.1,
  ...
  transition={{
    duration: 4 + i * 0.2, // setiap tombol beda timing dikit
    repeat: Infinity,
    ease: "easeInOut",
  }}
>
  ...
  {showInfoId === mission.id && (
    ...
    >
      ...
      <p className="leading-snug">{mission.info}</p>
        ...
        {/* 🌀 Pusat Portal */}
        <motion.div
          className="absolute left-[35%] top-[30%] -translate-x-1/2 -translate-y-1/2 w-28 h-28 md:w-36 md:h-36 rounded-full border border-yellow-400 bg-black/40 text-amber-300 font-bold text-sm md:text-base flex items-center justify-center text-center animate-pulse shadow-[0_0_25px_rgba(251,191,36,0.4)] backdrop-blur-sm"
          ...
          style={{
            textShadow:
              "0 0 8px rgba(251,191,36,0.5), 0 0 20px rgba(251,191,36,0.2)",
          }}
        >
          🌀 PILIH MISI
        </motion.div>
